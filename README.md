## 職務経歴書(AIエンジニア)
作成：2026年6月5日 | 氏名：鴇田　優太 | 年齢：24歳

## 職務概要

　2024年3月に法政大学を卒業後、千葉銀行にAI/MLエンジニアとして入行。２年に渡り、生成AI(LLM)と画像認識を活用した業務効率化及び自動化の内製開発プロジェクトを、ヒアリングからモデル開発、Google Cloud上でのアプリケーション運用まで一貫して主導。2025年12月より、グループ会社のエッジテクノロジー株式会社へDXトレーニー（在籍出向）として参画し、クライアント先の案件に従事。

主な実績と専門性：

- **LLM**：LangGraphを活用したRAGシステムやAIエージェントを設計・開発。行内文書のセマンティック検索を活用したQAアシスタント、自動的な文書振り分け、マッチングシステム等を実現し、部門の業務効率化に貢献。
- **画像認識**:デジタルサイネージ広告の効果測定のため、物体検知・追跡ソリューションをローカルPC上で開発。交通人数・視聴人数・属性データの分析により、2026年の大型サイネージ拡大に向けた、展開店舗の選定や広告価値の可視化といった経営判断を支援。
- **専門性・クラウド**：Googleの[Advanced Solutions Lab](https://cloud.google.com/customers/chiba-bank?hl=ja)を修了。複数のAWS、Google Cloudの認定資格を取得。学生時代から取り組んでいるKaggleやSIGNATEで培った実装力やドメインをビジネス課題に応用。

---

## 職務経歴(所属企業)

| 期間 | 企業 / 組織 | 役割 / プロジェクト |
| :--- | :--- | :--- |
| 2024/04 – 2024/10 | **株式会社千葉銀行(正社員)**<br>デジタル戦略部 | **OJT研修**<br>業務改革、デジタルチャネル、マーケティンググループの3領域を経験。部内のDX業務の全体感を理解。 |
| 2024/10 – 2024/11 | **Google(研修)**<br>[Advanced Solutions Lab](https://cloud.google.com/customers/chiba-bank?hl=ja) | **ML Immersive Education研修**<br> TensorFlowを用いたMLの理論と実装を習得。日本手話の認識プロトタイプを開発。|
| 2024/11 – 2025/12 | **株式会社千葉銀行(正社員)**<br>デジタル戦略部 AIソリューション室 | **AI/ML エンジニア**<br> RAG、AIエージェント、画像認識を用いたプロジェクトを実装から運用まで一貫で主導。|
| 2025/12 – 現在 | **エッジテクノロジー株式会社(DXトレーニー: 千葉銀行より在籍出向)**<br>イノベーション推進室 | **AI/ML エンジニア・データサイエンティスト**<br>受託開発の企業にて、大手企業向けのプリセールスPoCやBQMLを用いたコスト最適化や検索システムのモデル改善PoCに従事。 |

### DXトレーニーの背景と役割
　千葉銀行では内製開発で、一貫して開発から運用まで担当。AIアルゴリズムを事業とするエッジテクノロジーにて、本場のAIエンジニアからより高度な技術力を学ぶ、または実際にプロジェクトに参画して養うため。

---

## 職務経歴(プロジェクト詳細)

## 株式会社千葉銀行：_2024年4月~現在_

| 項目 | 内容 |
| :--- | :--- |
| **企業名** | 株式会社千葉銀行 (The Chiba Bank, Ltd.) |
| **在籍期間** | 2024年4月 – 現在 |
| **事業内容** | 普通銀行業務（地方銀行） |
| **上場区分** | 東証プライム上場 |
| **資本金** | 1,450億円 (2025年3月末時点) |
| **売上高** | 3,621億7,900万円 (2025年3月期) |
| **従業員数** | 4,076名 |


### 1. 本部の企業サポート部向け　融資決裁権限に関する問い合わせアシスタントAIの開発
- **期間**：2024年11月~2025年3月（5カ月）
- **体制**：エンジニア2名
- **対象**：企業サポート部の問い合わせ担当者5名
- **ビジネス課題**：月80件に及ぶ融資決裁権限に関する問い合わせ対応の負荷軽減と回答品質の標準化

#### 背景概要
　銀行では、融資実行の可否を決定する**決裁権限**に関するマニュアルがあり、融資案件が支店長等の裁量内に該当するか、もしくは本部への正式な稟議が必要かを詳述している。融資には細心の注意が必要な上、こちらのマニュアルを読んでも解決しないケースが多いため、本部担当者への問い合わせがチャットボットを通じて頻発していた。開発にあたって、業務側からは、マニュアルに記載のない細かな判断基準や回答のニュアンスを維持したいという要望があった。

#### 役割、アクション
- **データ前処理と匿名化**：過去5,000ペアのQAセットから個人情報をマスキングし、CSV形式（項目、質問、回答）へ整形。回答列を「回答・条件・参照」に構造化し、LLMが解釈しやすいフォーマットを定義。
- **RAG基盤の構築**：ChromaDBを用いたベクトルデータベースおよびLangChainによるRAGシステムを設計。
- **Human-in-the-Loop(HITL)の実装**：AIの回答をそのまま出力せず、人間が検証・修正を行うプロセスを確立。評価や責任の分離のためだけでなく、新規の質問に対するAIと人間の最終回答をペアで保管・蓄積するデータパイプラインで運用中。
- **学習サイクルの担当**：回答品質を継続的に向上させるため、蓄積された正解データを用いて定期的にベクトルデータベースを更新。
- **UI開発**：ipywidgetsを用いたアプリケーションUIの構築。
  
#### 成果
　担当者評価でAccuracy80%を達成。検索時間の短縮に加え、熟練者の言葉遣いを参考にした回答が可能になり、ナレッジの平準化と工数削減に貢献。


#### Technical Stack
RAG, LangChain, ChromaDB, ipywidgets

### **2. 市場業務部向け　国際収支項目番号検索AI(外国送金マニュアルのRAGシステム)**

- **期間**：2024年11月～2025年3月（5カ月）
- **体制**：エンジニア2名
- **対象**：市場業務部の行員（8名程度）
- **ビジネス課題**：80ページに及ぶ日本銀行発行マニュアルの参照工数の削減、および本部への業務集中に伴う負荷軽減

#### **背景概要**
　市場業務部では、送金目的に応じて指定される、3桁の国際収支項目番号を日本銀行へ報告する必要がある外国送金業務を取り扱っており、発行された80ページものマニュアルを参照しており、多くの時間を要していた。中期経営計画で掲げる意思決定や情報の本部集中に伴う業務増に対応するため、新任スタッフでも、迅速に過去ケースやマニュアルを参照できる環境が必要であった。

#### **役割、アクション**
- **E2E**：業務課題のヒアリングから要件定義、開発、デプロイまで一貫して担当。
- **PDFパースの最適化**：マニュアルの階層構造に基づいたチャンク化を実施し、検索精度を向上。
- **トレーサビリティの確保**：回答の根拠となったPDFの該当箇所をStreamlit上で即座にプレビューできるメタデータ連携機能を実装。
- **コンテナ運用**：Google CloudのCloud Runを用いたサーバーレスなデプロイ・運用環境を構築。

#### **成果**
　担当者評価でAccuracy80%（Correct/Helpful/Incorrect指標）を達成。また、マニュアル参照時間を大幅に削減。

#### **Technical Stack**
Cloud Run, GCS, Streamlit, RAG, LangChain, ChromaDB

### **3. デジタル戦略部業務改革G向け　提案制度振り分けAIエージェント(RAG/AI-agent)**
- **期間**：2025年3月～2025年5月（3カ月）
- **体制**：エンジニア1名 
- **対象**：提案制度システム受付担当者(毎月2名)
- **ビジネス課題**：過去4,000件に及ぶ改善提案に対する重複判定と担当部署割り当て作業の効率化

#### **背景概要**
　デジタル戦略部業務改革グループでは、支店行員から日常業務の改善案を受け付けている掲示板を4年ほど運用しており、過去4,000件の提案がある。4年間の運用で蓄積された膨大なデータに対し、新しい提案の重複チェックや適切な対応先の判断が属人化し、処理工数がボトルネックとなっていた。
　
#### 役割、アクション
- **要件定義とプロセス設計**：業務改革グループへの直接ヒアリングを通じ、従前の判断フローをAIエージェントで代替。
- **マルチエージェントの実装**：LangGraphを活用し、重複判定と所管部振り分けを個別のエージェントに分担させるワークフローを構築。
- **データセットの整形と匿名化**：CSVカラムの整形と過去4,000件のデータ内の個人情報のマスキング
- **差分更新インデックスの実装**：担当者がUI上で新規提案を追加する際、ベクトルDBのインデックス全体を再構築するのではなく、追加された差分のみを処理する仕組みを実装し、データベース更新時の工数を最小化。
- **フルスタック開発**：Streamlitを用いたUI構築、Cloud Runへのデプロイ、GCSおよび一時データとの連携処理を完遂。

#### **成果**
　1件あたりの処理時間を平均15分から約1分へ短縮（93%削減）。年間約1,000件の提案に対し、約233時間の工数削減を達成。また、類似検索の観点で新規スタッフ層からの定性的なポジティブな評価を獲得。

#### **Technical Stack**
LangGraph LangChain, RAG, ChromaDB, GCS, Cloud Run, Artifact Registry, Streamlit

### **4. 広告出稿主向け　デジタルサイネージ広告効果測定AIの開発・実証実験**
- **期間**：2024年11月～2025年11月（1年）
- **体制**：エンジニア1名（設計・開発・運用を担当）
- **対象**：デジタル戦略部広告チーム、既存・新規の広告主
- **ビジネス課題**：2026年の大型デジタルサイネージの拡大に向けた広告効果（視聴数・属性・交通量）の可視化や他行との差別化等

#### **背景概要**
　オフラインのため、定量的な効果測定が困難であるデジタルサイネージ広告において、クライアント先へAIによる統計データを還元することで、既存顧客の継続および新規獲得を狙う。

#### **役割とアクション**
- **PoCの推進**：フェーズ1（交通量測定）からフェーズ2（視聴判定・属性分析）まで順次モデルを開発。
- **ハードウェア最適化**：YOLO, MediaPipe, DeepSORTを用いたモデルを構築。カメラ解像度、モデルサイズ、ローカルPC、エッジデバイス（NVIDIA Jetson）、クラウド環境を用いて、様々な計算リソース制限のもとで、精度と速度のトレードオフ検証を実施
- **AIガバナンスへの対応**：コンプライアンス・リスク統括部と連携し、弁護士相談を通じた個人情報保護観点の整理やリスク評価、PIIマスキング、および利用目的の策定など、法規制を遵守した運用スキームを確立。
- **他社ヒアリング等**

#### **成果**
　現在、フェーズ１の段階において、千葉駅前支店でのローカルPCを用いたPoCにより技術的実現性を証明。2026年の大型サイネージ展開店舗の選定や、オフラインデータに基づく広告価値提供の基盤を構築。

#### **Technical Stack**
YOLO, MediaPipe, Ultralytics, DeepSORT, PINTO_Model, NVIDIA Jetson, OpenCV


#### 処理イメージ
[処理イメージ(実際は、統計情報として月間のレポートを作成・提供予定)](https://youtu.be/rSKJr9EE3gk4)
![ads_img_1](/assets/img/ads_img_1.png)<br>

<!-- <video width="100%" controls muted>
  <source src="https://github.com/user-attachments/assets/cfca01e4-8313-44cd-ac9f-45480b2e01ec?raw=true" type="video/mp4">
</video> -->

## **株式会社エッジテクノロジー(在籍出向中)：_2025年12月~現在_**

| 項目 | 内容 |
| :--- | :--- |
| **企業名** | 株式会社エッジテクノロジー |
| **在籍期間** | 2025年12月 – 現在 |
| **事業内容** | AIアルゴリズム事業 |
| **上場区分** | 上場廃止(2024年12月にTOB後に千葉銀行グループ完全子会社 |
| **従業員数** | 99名(2025年6月時点) |


### **5. 大手飲料メーカー向け BigQueryを用いたブランドマスタ紐付けPoC(BQML/RAG)**
- **期間**：2025年12月~2026年1月（2カ月）
- **体制**：エンジニア1名、PM1名
- **対象**：大手飲料メーカー
- **ビジネス課題**：既存システムのインフラ複雑性の排除、運用コスト・開発工数の削減（実現可能性の検証）

#### **背景概要**

　顧客先では、Google Cloud上でCloud FunctionやWorkflowsなど用いたマスタの紐付けシステムにおけるインフラの複雑性や開発・運用コストを懸念している。SQLベースでAI機能を利用できるBQMLや定期スケジュール実行などのBig Query内部機能を用いることで、従前のシステムの複雑性を排除したRAGベースの商品名とブランド名の紐付けタスクの実現可能性を検証、提案するため本PoCに着手。

#### **役割、アクション**
- Google Cloud StorageからBigQueryへのデータのロード
- データ前処理：商品名の略称に対応するための拡張データを作成。NFCK正規化や文字列置換、ユニーク化を行うUDF（ユーザー定義関数）を実装し、AI処理前に商品名のユニーク数を80%削減
- AI処理前の完全一致結合
- Remote model機能を用いて、埋め込みモデルであるgemini-embedding-001で商品名とブランド名をベクトル化したテーブルの作成
- ベクトル検索で算出された、コサイン距離の近い上位５件をプロンプトに組み込ませ、テキスト生成モデルであるgemini-flash-2.5でブランド名を判断理由とともにJSON形式で推定(デフォルトでバッチ処理）
- プロシージャとして上記の一連のSQLをワークフローとして定義し、定期実行スケジュールの実証。
  
#### **成果**
1,500件の商品データに対して正解率80%、実行時間1分半、コスト数十円といったパフォーマンスを実証。また、BigQueryのみでRAGを実現することでインフラ管理不要化による運用工数の削減を証明。

#### **Technical Stack**
SQL, BQML, Google Cloud, Big Query

### **6. 大手飲料メーカー向け　酒分類・ブランドマッチングモデルの精度改善PoC(NLP/LLM)**
- **期間**：2026年3月~現在
- **体制**：エンジニア1名、PM1名
- **対象**：大手飲料メーカー
- **ビジネス課題**: 本番運用して４年が経過し、精度が低下したレガシーなマッチングモデルの刷新と新規本契約の獲得

#### **背景概要**
　本システムは、レシートの商品情報からブランド名を推定しマスタとの結合を行うシステムであり、現在本番運用中で、3,4年が経過している。モデルの再学習は行っているが、精度が落ちてきているため、モデルの改善などを行って改善が見込まれた場合、本番契約の提案を行うプリセールスPoC。

#### **役割、アクション**
- 既存システムの理解(Bert分類モデル＋SentenceTransformerを用いた対照学習によるブランド検索モデル）と誤判定分析
- アーキテクチャやプロセスの変更後、推論速度、コスト、精度を検証
- 提案書の作成
  
#### **成果**
現在進行中

#### **Technical Stack**
Sentence Transformer, MultipleNegativesRankingLoss

---

## **Certificate**[Credly Profile](https://www.credly.com/users/yuta-tokita)

* **AWS**
  * **AWS Certified AI Practitioner** (*2025/02*)
  * **AWS Certified Solutions Architect – Associate** (*2024/07*)
  * **AWS Certified Cloud Practitioner** (*2024/06*)

* **Google Cloud**
  * **Professional Cloud Architect** (*2025/05*)
  * **Professional Machine Learning Engineer** (*2024/11*)
  * **Advanced Solutions Lab ML Immersive Education** (*2024/10*)
  * **Associate Cloud Engineer** (*2024/07*)

---

## **Skills**

#### **Machine Learning & Deep Learning**
* **Frameworks:** PyTorch, TensorFlow, Keras, JAX/Flax, ONNX, Mediapipe, OpenCV, Ultralytics, Firebase AI Logic
* **Computer Vision:**
    * 画像分類 (timm, TTA, TTAch)
    * 物体検出 (YOLO series, Mediapipe, PINTO-model)
    * 物体追跡 (Centroid, DeepSORT)
* **Generative AI:** Vertex AI, LangChain/LangSmith, Google ADK (Agent Development Kit), LangGraph, ローカルLLMの事後学習
* **Data Science:** LightGBM, CatBoost, XGBoost, 交差検証 (Group-KFold等), チューニング (Optuna等), 各種アンサンブル手法

#### **Data Engineering & Languages**
* **Languages:** Python, SQL
* **Data Warehouse:** BigQuery ML (BQML)

#### **Design/Frontend**
* WordPress, FastAPI x Streamlit, Slack

---

## **Competition/Awards**

#### **SIGNATE**
称号：**Master** (🥇1 | 🥈1 | 🥉4)<br>
順位：200,000人中 **85位** / [プロフィール](https://user.competition.signate.jp/ja/user/?user=9a0d3d95367d4fd98e680d58a487a4f5)<br>

* [テクノプロ・デザイン社 食品パッケージ画像解析チャレンジ](https://signate.jp/competitions/1106)：**Solo Gold🥇**
![signate_1](/assets/img/signate_1.png)
  
---

## **講演・講義**
* [Introduce Data Science & Econometrics Laboratory in Hosei Univ.(Youtube)](https://www.youtube.com/watch?v=E-qVjWBCrug&t=257s)
  * ゼミ長として、劉研究室の紹介を担当
![intro labs](/assets/img/intro_labs.png)<br>

* [AI-agent 実践集中コース Linghtning Talks hosted by Google.](https://youtu.be/d6A4VnyZTk4)
  * 内製開発プロジェクト3の提案制度振り分けAIエージェントでのポイントを共有
![LT_1](/assets/img/LT_1.png)<br>

* [Lightning Talks in AI Dojo hosted by Google.](https://docs.google.com/presentation/d/1j2_xmHDAVeNJg4vjDoY1NerwFUJU-mAxOWnhZ-fd8Ho/edit?usp=sharing)
  * Google AI Dojo で学んだFirebase を利用することで、例えばWordPress で作成した自分のWebブログにAI機能をセキュアに簡単に組み込めることを紹介
  * Innovation賞、優秀賞、Speed賞を受賞
![LT_2](/assets/img/LT_2.png)<br>

---

## **出版**
1.汎化性能を考慮した食品パッケージの画像分類, 鴇田優太, (2024)<br>
[Abstract](/assets/img/20X4110-0.pdf) [Paper](/assets/img/20X4110-1.pdf) [Slide](/assets/img/20X4110-2.pdf).

---

## **社外活動**
- 松尾研DL/LLMコミュニティ所属

- [Deep Learning 基礎講座 2025 Autumn](https://weblab.t.u-tokyo.ac.jp/lecture/deep-learning/): [修了証](/assets/img/DL2025.pdf)（最終課題の[Visual Question Answering](https://vizwiz.org/tasks-and-datasets/vqa/)コンペで8位）
- [大規模言語モデル基礎編 2025](https://weblab.t.u-tokyo.ac.jp/large-language-model/)：[修了証](/assets/img/LLMbasic2025.pdf)
- [大規模言語モデル応用編 2025](https://weblab.t.u-tokyo.ac.jp/large-language-model-advanced-course/)：[修了証](/assets/img/LLMadvance.pdf)
  


